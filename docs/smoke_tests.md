...
| S-V8-MENU-02 | Abrir `Monitoramento Operacional` | Página read-only exibe Saúde Técnica, WhatsApp/Meta, Auditoria, Eventos (`?view=events`), Diagnóstico/Readiness e Health/Runtime |
...
| S-V8-MENU-04 | Abrir sidebar como supervisor | Supervisão mostra um único filho `Central do Supervisor`; drill-downs internos preservam Backoffice, Qualidade, SLA (`?sla=risk`), Alertas IA e Inatividade/Autoclose (`?inactivity=autoclose_done`) |
