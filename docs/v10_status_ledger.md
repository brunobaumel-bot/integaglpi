...
- Status: **G2 `FIXED_PENDING_CURSOR_007`** (`g2_outbound_null_ready=false`; construction bloqueada).
...
- Build subset congelado: `build_subset_hash=baafb11eb04a956eeaca0d0e332e115c21772f79b0c63bcc49bca093c467f25f`; local e HML calcularam o mesmo valor com o mesmo script. Subset = package/package-lock/tsconfig/shadow tsconfig/Dockerfile/ignore dedicado/`src/shadowReplay/**`.
- Hashes antigos de build subset (`5a68…`, `b661…`, `122b…`, `0340…`) foram superseded por terem sido gerados com algoritmos, raízes ou manifestos diferentes; não são estado atual.
...
- `named_ignore_exercised=false`; `full_context_ignore_exercised=false`; full-context ignore/canary permanece para smoke HML pós-commit.
