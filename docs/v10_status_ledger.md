...
### G3 Shadow Store schema/contract — 2026-06-22

Phase: `integaglpi_v10_shadow_replay_lab_g3_shadow_store_schema_contract_001`.

- Status: **G3 `IMPLEMENTED_PENDING_CURSOR_REVIEW`** (`g3_shadow_store_contract_created=true`; `shadow_replay_runtime_allowed=false`; produção bloqueada).
- Migration versionada, não aplicada: `integration-service/schema-migrations/061_shadow_replay_store.sql`.
- Tabelas contratuais isoladas: `shadow_replay_runs`, `shadow_replay_samples`, `shadow_replay_results`, `shadow_replay_audit_events`.
- Schema aditivo: `CREATE TABLE IF NOT EXISTS`/`CREATE INDEX IF NOT EXISTS`; sem `ALTER`, `INSERT`, `UPDATE`, `DELETE`, `TRUNCATE` ou `DROP`; sem trigger em tabela operacional; sem FK para registros operacionais.
- Dados permitidos: hashes/referências sintéticas, timestamps e JSONB sanitizado. Sem storage de corpo bruto, provider payload ou identificadores pessoais diretos.
- Contratos TS: `ShadowReplayStoreTypes.ts` e `ShadowReplayStoreContract.ts`; sem cliente de banco/cache, sem adapter real e sem implementação de escrita.
...
### G5 Shadow Store synthetic transaction smoke — 2026-06-23

Phase: `integaglpi_v10_shadow_replay_lab_g5_shadow_store_synthetic_transaction_smoke_001`.

- Status: **G5 `HML_SYNTHETIC_TRANSACTION_SMOKE_PENDING_CURSOR_REVIEW`**.
- Target HML: `glpi-integaglpi-postgres`; containers `glpi-integaglpi-prod-*` não foram usados.
- Smoke final: `BEGIN` → insert sintético em `shadow_replay_runs`,
  `shadow_replay_samples`, `shadow_replay_results` e
  `shadow_replay_audit_events` → select de validação → `ROLLBACK`.
- `COMMIT` usado: `false`; scan do SQL final para referências operacionais/PII/payload: `0`.
- Run sintética final: `shadow-smoke-20260623012338`.
- Metadata sintética: somente `{"synthetic": true, "phase": "g5"}`.
- Dentro da transação: quatro linhas visíveis (`1` em cada tabela shadow).
- Pós-rollback: quatro tabelas com `0` linhas; nenhum dado sintético persistiu.
- Escopo DB: somente `public.shadow_replay_*`; nenhuma tabela operacional lida ou escrita.
- Segurança: sem produção, sem deploy, sem runtime Node/PHP, sem Redis/FSM, sem
