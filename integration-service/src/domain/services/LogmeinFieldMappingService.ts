...
function extractLmValue(inventory: LogmeinHardwareInventory, fieldKey: string): string | null {
  switch (fieldKey) {
    ...
    case 'MemorySize':
      return inventory.memoryMb !== null ? String(inventory.memoryMb) : null;
    case 'CpuType':
      return inventory.processors[0]?.type ?? null;
    case 'CpuNumberOfCores':
      return inventory.processors[0]?.numberOfCores !== null
        ? String(inventory.processors[0]?.numberOfCores)
        : null;
    case 'CpuSpeed':
      return inventory.processors[0]?.speedMhz !== null
        ? String(inventory.processors[0]?.speedMhz)
        : null;
    case 'DriveCapacity':
      return inventory.drives[0]?.capacityMb !== null
        ? String(inventory.drives[0]?.capacityMb)
        : null;
    case 'DriveName':
...
export class LogmeinFieldMappingService {
  ...
  public async dryRun(input: {
    ...
    for (const mapping of mappings) {
      ...
      const safeProposed =
        logmeinFieldKey === 'NetworkConnectionMacAddress' || logmeinFieldKey === 'NetworkConnectionIPAddress'
